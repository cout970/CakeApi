...
public class ApiCommand implements CommandExecutor {
    ...
    public void execute(CommandSender commandSender, String[] args) {
        try {
            command.execute(CakeApiCore.getServer(), PluginInterface.getCommandSender(commandSender), args);
    ...
    public List<String> addTabCompletionOptions(CommandSender sender, String[] args, Vector3 pos) {
        return command.getTabCompletionOptions(CakeApiCore.getServer(),PluginInterface.getCommandSender(sender), args,
                PluginInterface.getBlockPos(pos));
    }

    @Override
    public boolean canBeUsedBy(CommandSender commandSender) {
        return command.checkPermission(CakeApiCore.getServer(),PluginInterface.getCommandSender(commandSender));
    }
}
